function eggnstone_widgets_fill_amazon_search_box_for_search_forms(index, url, keywords)
{
...
function eggnstone_widgets_fill_amazon_search_box(index, url, keywords)
{
    const element = document.getElementById("amazon-search-" + index);
    ...
    req.addEventListener("load", function ()
    {
        if (req.status === 200)
        {
            ...
            if (!items || items.length === 0)
                element.innerHTML = eggnstone_widgets_create_search_box_table_with_message(index, url, keywords, "Nothing found.");
            else
                element.innerHTML = eggnstone_widgets_create_search_box_table_with_result(index, url, keywords, items, maxTitleWidth);
        }
        else
            element.innerHTML = eggnstone_widgets_create_search_box_table_with_message(index, url, keywords, "Nothing found.");
    });

    element.innerHTML = eggnstone_widgets_create_search_box_table_with_message(index, url, keywords, "Loading ...");

    req.send(null);
}

function eggnstone_widgets_refill_amazon_search_box(index, url)
{
    const keywords = document.getElementById("amazon-search-input-" + index).value;
    eggnstone_widgets_fill_amazon_search_box(index, url, keywords);
}

function eggnstone_widgets_on_search_button_click(index, url)
{
    eggnstone_widgets_refill_amazon_search_box(index, url);
}

function eggnstone_widgets_on_search_text_key_up(key, index, url)
{
    if (key === "Enter")
        eggnstone_widgets_refill_amazon_search_box(index, url);
}

function eggnstone_widgets_create_search_box_table_with_message(index, url, keywords, message)
{
    let html = eggnstone_widgets_create_search_box_table_start(index, url, keywords);
    ...
    html += eggnstone_widgets_create_search_box_table_end();

    return html;
}

function eggnstone_widgets_create_search_box_table_with_result(index, url, keywords, items, maxTitleWidth)
{
    let html = eggnstone_widgets_create_search_box_table_start(index, url, keywords);
...
function eggnstone_widgets_create_search_box_table_start(index, url, keywords)
{
    return "" +
        ...
        "          <td class='left-cell'>" +
        "            <label>" +
        "              <input id='amazon-search-input-" + index + "' value='" + keywords + "' onkeyup='eggnstone_widgets_on_search_text_key_up(event.key, " + index + ", \"" + url + "\");'/>" +
        "            </label>" +
        "          </td>" +
        "          <td class='right-cell'>" +
        "            <button onclick='eggnstone_widgets_on_search_button_click(" + index + ", \"" + url + "\");'/>" +
...
function eggnstone_widgets_create_search_box_table_end()
{
    return "</table>";
}
